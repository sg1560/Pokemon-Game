...
npc_t *characterMap[MAP_Y][MAP_X];
...
static int32_t player_cmp(const void *key, const void *with) {
  printf("UGH");
  return (characterMap[((npc_t *) key)->pos[dim_y]]
                          [((npc_t *) key)->pos[dim_x]] ->gameTurn -
          characterMap[((npc_t *) with)->pos[dim_y]]
                          [((npc_t *) with)->pos[dim_x]] ->gameTurn);
}
static void movePacer(npc_t *pacer){
 int x = pacer->pos[dim_x];
 int y = pacer->pos[dim_y];
 terrain_type_t terrain = world.cur_map->map[y][x + 1];
 int cost = 0;

 if(terrain == ter_path || terrain == ter_clearing){
  cost = 10;
 ...
 if(x + 1 < MAP_X){
  ...
  //update the pacer's position
  pacer->pos[dim_x] = x + 1;
  //update the pacer's game turn
  pacer->gameTurn = pacer->gameTurn + cost;
  //update the character map
  characterMap[y][x] = NULL;
  characterMap[y][x+1] = pacer;
 }

}
static void makeMove(npc_t *character){
  // switch(character->character){
  //   case(char_pacer):
  //     return movePacer(character);
  // }
  movePacer(character);
}
static void player_movement(){
  ...
  heap_init(&h, player_cmp, NULL);


  //This will contain the contents of the removed element from the heap
  npc_t *removedCharacter;
  //insert characters in heap
  world.pc.hn = heap_insert(&h, &world.pc);
  for(int y = 0; y < MAP_Y; y++){
    for(int x = 0; x < MAP_X; x++){
      if(characterMap[y][x] != NULL){
        characterMap[y][x]->hn = heap_insert(&h, characterMap[y][x]);
      }else{
        characterMap[y][x]->hn = NULL;
      }
    }
  }

  while((removedCharacter = heap_remove_min(&h))){
    if(removedCharacter->character == char_pacer){
       //the character will make it's move
      makeMove(removedCharacter);
      //insert back into heap
      heap_insert(&h, removedCharacter);
    }
  }
}
  ...
  for (y = 0; y < MAP_Y; y++) {
    for (x = 0; x < MAP_X; x++) {
      if (world.pc.pos[dim_y] == y &&
        ...
        putchar('@');
      } else if(characterMap[y][x] != NULL){
        switch(characterMap[y][x]->character){
          case char_pc:
            break;
...
void init_npc(character_type_t c, int xPos, int yPos){
  //create 
  npc_t *newNpc = malloc(sizeof(npc_t));
  newNpc->character = c;
  newNpc->pos[dim_y] = yPos;
  newNpc->pos[dim_x] = xPos;
  newNpc -> gameTurn = 0;
  characterMap[yPos][xPos] = newNpc;
}

void spawn_npc(){
  //generate the number of trainers from 10 to 20
  int numTrainers = (rand() % 11) + 10;
  //lopp through number of trainers, creating a new npc each time
  for(int i = 0; i < numTrainers; i++){
    int character = (rand() % 7) + 1;
    character_type_t characterType = character;
    ...
    //         break;
    //       case char_hiker:
    //         printf("%c", HIKER_SYMBOL);
    //         break;
    //       case char_rival:
    //         printf("%c", RIVAL_SYMBOL);
    //         break;
    //       case char_other:
    //         printf("%c", OTHER_SYMBOL);
    //         break;
    //     }
    ...
    do{
       x = rand() % (MAP_X - 2) + 1;
       y = rand() % (MAP_Y - 2) + 1;
    }while(world.cur_map->map[y][x] != ter_path && move_cost[characterType][world.cur_map->map[y][x]] && 
          characterMap[y][x] != NULL);

    //once we've found valid x and y coordiantes, we initailize the npc
    init_npc(characterType, x, y);
  }
