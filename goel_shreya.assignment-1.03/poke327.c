...
int getRivalCost(int yCoor, int xCoor, map_t *m){
  switch (m->map[yCoor][xCoor]){
    case ter_path:
      return 10;
    case ter_mart:
...
void printCostMap(path_t map[MAP_Y][MAP_X]){
  for(int y = 0; y < MAP_Y; y++){
    for(int x = 0; x < MAP_X; x++){
      printf("%d ", map[y][x].cost % 100);
    }
    printf("\n");
  }
}
...
static void build_hiker_map (pair_t from, map_t *map){
  ...
  //if the terrain cost isn't infinity, we add the heap node to the heap; otherwise set heap node to null
  for (int y = 1; y < MAP_Y - 1; y++) {
    for (int x = 1; x < MAP_X - 1; x++) {
      if(getCost(y, x, map) != SHRT_MAX){
         hikerMap[y][x].hn = heap_insert(&h, &hikerMap[y][x]);
      }else{
        hikerMap[y][x].hn = NULL;
      }
      //the heap node of every path_t cell is set to a pointer heap node
      //this pointer heap node contains data, which is the address of the path cell; this heap node is inserted in heap
      //so basically, every cell's heap node is in the heap as well, and the heap node contains the address of cell
  ...
  //8 if statements
    ...
    //2nd neighbor which is 1 to the left
     if ((hikerMap[p->pos[dim_y]][p->pos[dim_x] - 1  ].hn) &&
        (hikerMap[p->pos[dim_y]][p->pos[dim_x] - 1  ].cost >
         ((p->cost + getCost((p -> pos[dim_y]), p->pos[dim_x] - 1, map))))) {
        //update neighboring cell cost to current cell cost plus weight of edge
        hikerMap[p->pos[dim_y]][p->pos[dim_x] - 1  ].cost =
        ((p->cost + getCost((p -> pos[dim_y]), p->pos[dim_x] - 1, map)));
      
      //put neighboring cell's heap node back in heap
      heap_decrease_key_no_replace(&h, hikerMap[p->pos[dim_y]]
                                           [p->pos[dim_x] - 1 ].hn);
    }
    //3rd neighbor which is 1 to the right
    if ((hikerMap[p->pos[dim_y]][p->pos[dim_x] + 1  ].hn) &&
        (hikerMap[p->pos[dim_y]][p->pos[dim_x] + 1  ].cost >
         ((p->cost + getCost((p -> pos[dim_y]), p->pos[dim_x] + 1, map))))) {
        //update neighboring cell cost to current cell cost plus weight of edge
    ...
    //4th neighbor which is 1 to the bottom
    if ((hikerMap[p->pos[dim_y] + 1][p->pos[dim_x]    ].hn) &&
        (hikerMap[p->pos[dim_y] + 1][p->pos[dim_x]    ].cost >
         ((p->cost + getCost((p -> pos[dim_y] + 1), p->pos[dim_x], map))))) {
        //update neighboring cell cost to current cell cost plus weight of edge
  ...
  //5th neighbor which is 1 diagnoal northwest
    if ((hikerMap[p->pos[dim_y] - 1][p->pos[dim_x] - 1 ].hn) &&
        (hikerMap[p->pos[dim_y] - 1][p->pos[dim_x] - 1  ].cost >
         ((p->cost + getCost((p -> pos[dim_y] - 1), p->pos[dim_x] - 1, map))))) {
        //update neighboring cell cost to current cell cost plus weight of edge
        hikerMap[p->pos[dim_y] - 1][p->pos[dim_x] - 1   ].cost =
        ((p->cost + getCost((p -> pos[dim_y] - 1), p->pos[dim_x] - 1, map)));
    ...
    //6th neighbor which is 1 diagnoal northeast
    if ((hikerMap[p->pos[dim_y] - 1][p->pos[dim_x] + 1 ].hn) &&
        (hikerMap[p->pos[dim_y] - 1][p->pos[dim_x] + 1  ].cost >
         ((p->cost + getCost((p -> pos[dim_y] - 1), p->pos[dim_x] + 1, map))))) {
        //update neighboring cell cost to current cell cost plus weight of edge
    ...
    //7th neighbor which is 1 diagnoal southwest
    if ((hikerMap[p->pos[dim_y] + 1][p->pos[dim_x] - 1 ].hn) &&
        (hikerMap[p->pos[dim_y] + 1][p->pos[dim_x] - 1  ].cost >
         ((p->cost + getCost((p -> pos[dim_y] + 1), p->pos[dim_x] - 1, map))))) {
        //update neighboring cell cost to current cell cost plus weight of edge
    ...
    //8th neighbor which is 1 diagnoal southeast
    if ((hikerMap[p->pos[dim_y] + 1][p->pos[dim_x] + 1 ].hn) &&
        (hikerMap[p->pos[dim_y] + 1][p->pos[dim_x] + 1  ].cost >
         ((p->cost + getCost((p -> pos[dim_y] + 1), p->pos[dim_x] + 1, map))))) {
        //update neighboring cell cost to current cell cost plus weight of edge
  ...
  heap_delete(&h);
  printCostMap(hikerMap);
}

static void build_rival_map (pair_t from, map_t *map){
  //path is a static variable; it's a 2D array that contains path_t
  //*p is a pointer to type path_t; 
  static path_t rivalMap[MAP_Y][MAP_X], *p;
  static uint32_t initialized = 0;
  ...
  if (!initialized) {
    for (int y = 0; y < MAP_Y; y++) {
      for (int x = 0; x < MAP_X; x++) {
        rivalMap[y][x].pos[dim_y] = y;
        rivalMap[y][x].pos[dim_x] = x;
      }
    }
    initialized = 1;
  }
  ...
  for (int y = 0; y < MAP_Y; y++) {
    for (int x = 0; x < MAP_X; x++) {
      rivalMap[y][x].cost = SHRT_MAX;
    }
  }
  //Sets the cost of source node (pc) to 0
  rivalMap[from[dim_y]][from[dim_x]].cost = 0;
  ...
  for (int y = 1; y < MAP_Y - 1; y++) {
    for (int x = 1; x < MAP_X - 1; x++) {
      if(getCost(y, x, map) != SHRT_MAX){
         rivalMap[y][x].hn = heap_insert(&h, &rivalMap[y][x]);
      }else{
        rivalMap[y][x].hn = NULL;
      }
      //the heap node of every path_t cell is set to a pointer heap node
      //this pointer heap node contains data, which is the address of the path cell; this heap node is inserted in heap
      //so basically, every cell's heap node is in the heap as well, and the heap node contains the address of cell
  ...
  //8 if statements
    ...
    //if neighboring cell's heap node is not null (if it's in our heap)
    //AND the neighboring cell's cost is greater than our current cost plus the weight of our edge
    if ((rivalMap[p->pos[dim_y] - 1][p->pos[dim_x]    ].hn) &&
        (rivalMap[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost >
         ((p->cost + getRivalCost((p -> pos[dim_y] - 1), p->pos[dim_x], map))))) {
        //update neighboring cell cost to current cell cost plus weight of edge
        rivalMap[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost =
        ((p->cost + getRivalCost((p -> pos[dim_y] - 1), p->pos[dim_x], map)));
      
      //put neighboring cell's heap node back in heap
      heap_decrease_key_no_replace(&h, rivalMap[p->pos[dim_y] - 1]
                                           [p->pos[dim_x]    ].hn);
    }
    //2nd neighbor which is 1 to the left
     if ((rivalMap[p->pos[dim_y]][p->pos[dim_x] - 1  ].hn) &&
        (rivalMap[p->pos[dim_y]][p->pos[dim_x] - 1  ].cost >
         ((p->cost + getRivalCost((p -> pos[dim_y]), p->pos[dim_x] - 1, map))))) {
        //update neighboring cell cost to current cell cost plus weight of edge
        rivalMap[p->pos[dim_y]][p->pos[dim_x] - 1  ].cost =
        ((p->cost + getRivalCost((p -> pos[dim_y]), p->pos[dim_x] - 1, map)));
      
      //put neighboring cell's heap node back in heap
      heap_decrease_key_no_replace(&h, rivalMap[p->pos[dim_y]]
                                           [p->pos[dim_x] - 1 ].hn);
    }
    //3rd neighbor which is 1 to the right
    if ((rivalMap[p->pos[dim_y]][p->pos[dim_x] + 1  ].hn) &&
  ...
  //5th neighbor which is 1 diagnoal northwest
    if ((rivalMap[p->pos[dim_y] - 1][p->pos[dim_x] - 1 ].hn) &&
        (rivalMap[p->pos[dim_y] - 1][p->pos[dim_x] - 1  ].cost >
         ((p->cost + getRivalCost((p -> pos[dim_y] - 1), p->pos[dim_x] - 1, map))))) {
        //update neighboring cell cost to current cell cost plus weight of edge
        rivalMap[p->pos[dim_y] - 1][p->pos[dim_x] - 1   ].cost =
        ((p->cost + getRivalCost((p -> pos[dim_y] - 1), p->pos[dim_x] - 1, map)));
