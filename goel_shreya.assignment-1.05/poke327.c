...
static char *directions[] = {"north", "south", "east", "west"};
...
int check_move(int x, int y){
  ...
  if(world.cur_map->cmap[y][x] == NULL && t != ter_gate && t != ter_boulder && t != ter_tree){
    return 1;
  }

  //check if new location is a gate or not; pc cannot stand on gate
  return 0;
}

void make_move(pair_t move){
  int newX = world.pc.pos[dim_x] + move[0];
  int newY = world.pc.pos[dim_y] + move[1];
  if(check_move(newX, newY)){
    //erase the pc current location from the character map
    world.cur_map->cmap[world.pc.pos[dim_y]][world.pc.pos[dim_x]] = NULL;
    //set new location to hold the pc
    world.cur_map->cmap[newY][newX] = &world.pc;
    //set the position to the updated one
...
void check_trainer(int x, int y){
  if(world.cur_map->cmap[y][x] != NULL && world.cur_map->cmap[y][x]->symbol != '@'){
    int xDistance = x - world.pc.pos[dim_x];
    int yDistance = y - world.pc.pos[dim_y];
    
    printw(" %c: ", world.cur_map->cmap[y][x]->symbol);
    
    if(xDistance < 0){
      printw(" %d %s, ", abs(xDistance), directions[3]);
    }else{
      printw(" %d %s,", xDistance, directions[2]);
    }

    if(yDistance < 0){
      printw(" %d %s ", abs(yDistance), directions[0]);
    }else{
      printw(" %d %s ", yDistance, directions[1]);
    }
...
void game_loop()
{
  ...
  while (!quit) {
      ...
      switch(key){
        ...
        case '7':
          //make move;
          make_move(all_dirs[0]);
          break;
        case 'k':
        case '8':
          make_move(all_dirs[3]);
          break;
        case 'u':
        case '9':
          make_move(all_dirs[5]);
          break;
        case 'l':
        case '6':
          make_move(all_dirs[6]);
          break;
        case 'n':
        case '3':
          make_move(all_dirs[7]);
          break;
        case 'j':
        case '2':
          make_move(all_dirs[4]);
          break;
        case 'b':
        case '1':
          make_move(all_dirs[2]);
          break;
        case 'h':
        case '4':
          make_move(all_dirs[1]);
          break;
        case 't':
          trainerList = 1;
          display_trainers();
      ...
      //move the npcs after a pc has been moved
